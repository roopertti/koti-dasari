...
const stop = startScheduler({ db, apiKey, defaults });

console.log(
  `[transport] Worker started (default lat=${defaults.homeLatitude}, lon=${defaults.homeLongitude}, radius=${defaults.transportRadius}m, interval=${defaults.transportIntervalMs}ms)`,
);
